...
class ReportRow {
  ...
  pd.PdfPage addRowToPage(pd.PdfPage page, double offsetT,
    ...
    for (var i = 0; i < _rowElements.length; i++) {
      ...
      if (el is String) {
        page.graphics.drawString(el, repFont.font,
            bounds: Rect.fromLTRB(offX, offsetT, 0, 0));
      }

      if (el is ReportTitle) {
        page.graphics.drawString(el.title, el.font,
            bounds: Rect.fromLTRB(offX, offsetT, 0, 0));
        offsetT += 25;
      }
      ...
      if (el is pd.PdfBitmap) {
        page.graphics.drawImage(
            el,
            Rect.fromLTWH(
                offX, offsetT, (el.width as double), (el.height as double)));
      }
      ...
      if (el is TitledBitmap) {
        ...
        for (var i = 0; i < el.titles.length; i++) {
          ...
          page.graphics.drawString(title, repFont.getFont(size: 48),
              bounds: Rect.fromLTRB(
                  offX + el.img.width / 2 - (el.getTitleWidth(i) / 2),
                  offsetT + offT,
                  0,
                  0),
        ...
        page.graphics.drawImage(
            el.img,
            Rect.fromLTWH(offX, offsetT + offT, (el.img.width as double),
                (el.img.height as double)));
      }
...
class ReportFont {
  ...
  Future<void> init() async {
    fontData = (await rootBundle.load("fonts/RobotoMono-Regular.ttf"))
        .buffer
        .asUint8List();
...
class Report {
  ...
  pd.PdfDocument columnToPage(pd.PdfDocument pdfDoc, {pd.PdfBitmap? logo}) {
    ...
    for (var row in _rows) {
      ...
      totalWidth =
          row.getSize().width > totalWidth ? row.getSize().width : totalWidth;
    }
    totalWidth += pdfDoc.pageSettings.margins.left +
    ...
    if (logo != null) {
      double newHeight = 50;
      double newWidth =
          (newHeight / (logo.height as double)) * (logo.width as double);
      page.graphics.drawImage(
          logo,
          Rect.fromLTWH(
              totalWidth - 300, totalHeight - 100, newWidth, newHeight));
    }
